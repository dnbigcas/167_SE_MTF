% Quick and Dirty MTF Measurement (ROI + Multiple Band Averaging + Noise Reduction)
clear;
...
%% --- Step 5: Average ESF, apply smoothing, then LSF & MTF ---
esf_avg = mean(all_esf, 1);

% 🔹 Noise reduction: smooth ESF with Gaussian filter
esf_smooth = smoothdata(esf_avg, 'gaussian', 15);

lsf_avg = diff(esf_smooth);
...
%% --- Step 7+8: Overlay ESF (smoothed) and LSF ---
figure;
yyaxis left
plot(esf_smooth, 'b-', 'LineWidth', 1.5, 'DisplayName','Smoothed ESF');
ylabel('Intensity (a.u.)');

yyaxis right
plot(lsf_avg, 'r-', 'LineWidth', 1.5, 'DisplayName','LSF');
ylabel('dI/dx (a.u.)');

xlabel('Pixel position');
title('Edge Spread Function (ESF) and Line Spread Function (LSF)');
legend({'Smoothed ESF','LSF'}, 'Location','best');
grid on;
